...
$error = '';
$result = '';
// + Chỉ xử lý submit form nếu user  click nút Submit, dựa vào name của nút submit
// Sử dụng hàm isset để kiểm mảng có tồn tại phần tử theo key cho trước hay ko
if (isset($_POST['info'])) {
    // + Tạo và gán biến trung gian để thao tác cho dễ
    $name = $_POST['name'];
    // - Chú ý: với radio và checkbox, nếu ko tích chọn thì PHP sẽ ko bắt được dữ liệu gửi lên
    // Nên khai báo 2 biến $gender và $jobs như sau là ko chắc chắn
    // - Khi xử lý radio và checkbox, luôn cần check nếu tồn tại thì mới thao tác đc -> isset
//    $gender = $_POST['gender'];
//    $jobs = $_POST['jobs'];
    ...
    if (empty($error)) {
        ...
        switch ($country) {
            case 0:
                $result .= "Quốc gia: VN <br />";
                break;
            case 1:
                $result .= "Quốc gia: USA <br />";
                break;
            case 2:
                $result .= "Quốc gia: JP <br />";
                break;
        }
        // Xử lý textarea
...
<form action="" method="POST">
    ...
    Chọn nghề nghiệp:
    <?php
    // Đổ lại dữ liệu cho checkbox cũng tạo các biến tương ứng giống radio
    // Checked, sử dụng hàm in_array để ktra giá trị có nằm trong mảng hay ko
    $checked_dev = '';
    $checked_tester = '';
    $checked_ba = '';
    if(isset($_POST['jobs'])){
    ...
    ?>
    <input type="checkbox"
           name="jobs[]" value="0" <?php echo $checked_dev; ?>/>Dev
    <input type="checkbox"
           name="jobs[]" value="1" <?php echo $checked_tester; ?>/> Tester
    <input type="checkbox"
           name="jobs[]" value="2" <?php echo $checked_ba; ?>/> BA
    <br/>
    Chọn quốc gia:
    ...
    if (isset($_POST['country'])) {
        $country = $_POST['country'];
        switch ($country) {
            case 0: $country_vn = 'selected';break;
            case 1: $country_usa = 'selected';break;
            case 2: $country_jp = 'selected';
        }
    }
    ?>
    <select name="country">
        <option <?php echo $country_vn; ?> value="0">VN</option>
        <option <?php echo $country_usa; ?> value="1">USA</option>
        <option <?php echo $country_jp; ?> value="2">JP</option>
    </select>
    <br/>
    Ghi chú thêm:
