...
if (isset($_POST['submit'])) {
    ...
    if ($avatars['error'] == 0) {
        ...
        $extension = strtolower($extension); //chuyển về dạng chữ thường
...
//    var_dump(empty($error)); // true(1) -> $error rong
    ...
    if (empty($error)) {
        ...
        if ($avatars['error'] == 0) {
            ...
            $dir_upload = "uploads";
            // + Code để tạo thư mục, chứ ko tạo thủ công, cần check nếu đường dẫn thư mục
            // chưa tồn tại thì mới tạo: file_exists()
            if (!file_exists($dir_upload)) {
                // Tao thư mục: mkdir : make directory
                mkdir($dir_upload);
            }
            // Xử lý trùng file, tạo ra tên file mang tính duy nhất, sử dụng hàm time() để lấy
            //thời gian hiện tại: số giây tính từ thời điểm hiện tại so với mốc 01/01/1970
         //var_dump(time());
            $filename = time() . "-" . $avatars['name'];
            // + Upload file vào thư mục uploads: PHP chuyển file từ thư mục tạm -> thư mục
            ...
            if ($is_upload) {
                $result .= "Tên file sau khi upload: $filename <br />";
                $result .= "Đường dẫn tới file upload:  $dir_upload/$filename <br />";
                // Đuôi file
