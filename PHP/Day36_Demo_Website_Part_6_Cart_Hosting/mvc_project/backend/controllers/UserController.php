...
//            'query_additional' => $query_additional
        ...
        $pages = $pagination->getPagination();  // đổ ra phân trang
        $users = $user_model->getAllPagination($params); // mỗi trang
    ...
    public function update() {
        ...
        $id = $_GET['id'];
        $user_model = new User();
        if(empty($user_model->checkId($id))){
            $_SESSION['error'] = "Không có User nào có id = $id";
            header("Location: index.php?controller=user");
            exit();
        }
        $user = $user_model->getById($id); // lấy ra user của id hiện tại , tí còn xóa ảnh
        if (isset($_POST['submit'])) {
            $first_name = $_POST['first_name'];
            $last_name = $_POST['last_name'];
            $phone = $_POST['phone'];
